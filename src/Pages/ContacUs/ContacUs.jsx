...
const ContacUs = () => {
	...
	useEffect(() => {
		...
		if (response?.error?.data && response?.error?.data?.statusCode == 400) {
			Object.keys(response?.error?.data?.errors).forEach((key) => {
				response?.error?.data?.errors[key].forEach((msg) => {
					Alert({
						title: "Error",
						text: msg,
						iconStyle: "error",
									...
									<CommonInputField
										...
										onChange={(e) => {
											...
											if (regex.test(e.target.value)) {
												...
												setFormErrors({
													...formErrors,
													first_name: null, // Clear error
												});
											} else {
												setFormErrors({
													...formErrors,
													first_name: "Only letters are allowed in this field.",
												});
											}
										}}
										errors={
									...
									<CommonInputField
										...
										onChange={(e) => {
											...
											if (regex.test(e.target.value)) {
												...
												setFormErrors({
													...formErrors,
													last_name: null, // Clear error
												});
											} else {
												setFormErrors({
													...formErrors,
													last_name: "Only letters are allowed in this field.",
												});
											}
										}}
										errors={
