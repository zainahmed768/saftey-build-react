...
import { useFaqQuery } from "../../redux/services/FaqServices";
...
const Faq = () => {
  ...
  let faqsData = faqs?.response?.data;
  const [openIndex, setOpenIndex] = useState(0);

  const handleClick = (index) => {
    setOpenIndex(openIndex === index ? null : index);
  };
  console.log(faqsData?.length, "faqs?.length");
  return (
    <>
      <PrimaryHeader
        ...
        <div className="py-5 site_width">
          <div class="faqs__wrapp">
            <div class="row justify-content-center">
              <div class="p-2">
                {isLoading ? (
                  <div className="loading-wrap px-4">
                    <Skeleton />
                  </div>
                ) : faqsData?.length < 1 ? (
                  <h1 className="text-center">No Faqs Found</h1>
                ) : (
